...
export type FacetDef = { key: string; label: string; placeholder: string };
export type ActiveFilter = { facet: string; value: string };
...
export function FacetSearchBar({
    ...
    const dropdownItems: DropdownItem[] = React.useMemo(() => {
        ...
        return filtered;
    }, [parsed, suggestions, facets]);

    React.useEffect(() => { setHighlightedIdx(-1); }, [inputValue]);

    const commitFilter = React.useCallback((facet: string, value: string) => {
    ...
    const handleItemClick = React.useCallback((item: DropdownItem) => {
        if (item.kind === 'facet') {
            ...
            commitFilter(parsed.facet, item.value);
        }
    }, [parsed, commitFilter]);
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'ArrowDown') {
            ...
            if (highlightedIdx >= 0 && highlightedIdx < dropdownItems.length) {
                ...
                commitFilter(parsed.facet, parsed.query);
    ...
    return (
        <div className="relative flex-1" ref={containerRef}>
            <div
                className="flex flex-wrap items-center gap-1.5 rounded-lg border border-slate-700/60 bg-slate-900/60 px-3 py-1.5 min-h-[34px] transition-all focus-within:border-cyan-500/50 focus-within:ring-1 focus-within:ring-cyan-500/20 cursor-text"
