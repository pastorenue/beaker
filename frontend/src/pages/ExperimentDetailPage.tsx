...
import { OnePagerModal } from '../components/ai-assist/OnePagerModal';
import { aiStrategistApi } from '../services/api';
import type { Experiment, OnePagerDraft, UpdateExperimentRequest } from '../types';
...
export function ExperimentDetailPage() {
    ...
    const [onePagerDraft, setOnePagerDraft] = React.useState<OnePagerDraft | null>(null);
    ...
    const onePagerMutation = useMutation({
        mutationFn: () => aiStrategistApi.draftOnePager({ experiment_id: id! }),
        onSuccess: (response) => {
            setOnePagerDraft(response.data);
            setShowOnePager(true);
        },
        onError: () => addToast('Failed to generate one-pager', 'error'),
    });
        ...
        <div className="space-y-0">
            ...
            <div className="fixed bottom-6 right-6 z-30 flex flex-col gap-2">
                <button
                    ...
                    className="flex items-center gap-2 rounded-full bg-indigo-700 px-4 py-3 text-sm font-semibold text-white shadow-lg hover:bg-indigo-600 transition-colors disabled:opacity-50"
                >
                    Generate One-Pager{onePagerMutation.isPending ? '…' : ''}
                </button>
                <button
                    onClick={() => setIsAiDrawerOpen(true)}
                    className="flex items-center gap-2 rounded-full bg-gray-500 px-4 py-3 text-sm font-semibold text-white shadow-lg hover:bg-cyan-400 transition-colors"
                >
                    <svg
            ...
            {showOnePager && onePagerDraft && (
                <OnePagerModal draft={onePagerDraft} onClose={() => setShowOnePager(false)} />
            )}
        </div>
    );
}
